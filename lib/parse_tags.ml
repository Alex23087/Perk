...
(* TODO: Change the filename depending on current perk file being processed.
  Currently, only one pair of files is generated per compilation, even if multiple perk
  files are involved, with all but the last one simply being overwritten *)

(** Filename for the expansion of the C libraries included in the program. The
    filename is randomised for each execution to allow for concurrent
    compilation. *)
let libs_expanded_file =
  Filename.concat
    (Filename.get_temp_dir_name ())
    (Printf.sprintf "perkc_libs_expanded_%s.h" Utils.execution_UUID)

(** Filename for the ctags generated by the C libraries included in the program.
    The filename is randomised for each execution to allow for concurrent
    compilation. *)
let tags_file =
  Filename.concat
    (Filename.get_temp_dir_name ())
    (Printf.sprintf "perkc_tags_%s" Utils.execution_UUID)

let libs_expanded_file_was_generated = ref false
let tags_file_was_generated = ref false
...
let generate_tags lib_paths =
  ...
  in
  if status = 0 then (
    libs_expanded_file_was_generated := true;
    let status =
      Sys.command
        (Printf.sprintf "ctags --kinds-C=pt -f %s --fields=+Snt --sort=no %s"
           tags_file libs_expanded_file)
    in
    if status = 0 then tags_file_was_generated := true
    else
      raise
        (Errors.Compilation_error
           ( (-1, -1),
             (-1, -1),
             "Error while generating C tags",
             !Utils.fnm,
    ...
    raise
      (Errors.Compilation_error
         ( (-1, -1),
           (-1, -1),
           "Error while expanding C libraries",
           !Utils.fnm,
           Error_codes.External_error ))
...
let parse_line s =
  if String.starts_with ~prefix:"!" s then Comment
    ...
    if List.length splits < 4 then Other
    else
...
let remove_tags () =
  if !tags_file_was_generated && not !Utils.retain_tmp_files then
    let status = Sys.command (Printf.sprintf "rm %s" tags_file) in
    if status <> 0 then failwith "Couldn't remove file"
    else tags_file_was_generated := false

let remove_libs_expanded () =
  if !libs_expanded_file_was_generated && not !Utils.retain_tmp_files then
    let status = Sys.command (Printf.sprintf "rm %s" libs_expanded_file) in
    if status <> 0 then failwith "Couldn't remove file"
    else libs_expanded_file_was_generated := false
